...
#include "hw_ctl/diffbot_system.hpp"
...
void HardwareCommandPub::publishData(std::string msg)
{
  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  {
    // Error checking for the two states
...
std::vector<hardware_interface::CommandInterface> DiffBotSystemHardware::export_command_interfaces()
{
  ...
  return command_interfaces; // Twist command
}

hardware_interface::return_type DiffBotSystemHardware::start() // Initiate hardware
{
...
// Use this method to stop all hardware safely
...
// hardware_interface::return_type DiffBotSystemHardware::read() // Read the state from the robot hardware.
// {
//   RCLCPP_INFO(rclcpp::get_logger("DiffBotSystemHardware"), "Reading...");

//   double radius = 0.25;  // radius of the wheels
//   double dist_w = 0.41;   // distance between the wheels
...
hardware_interface::return_type ros2_control_demo_hardware::DiffBotSystemHardware::write() // Write the command to the robot hardware.
{
  ...
  hw_cmd_pub_->publishData(std::to_string(hw_commands_[0]) + " " + std::to_string(hw_commands_[1]));  //publish to topic
